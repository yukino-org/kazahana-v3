<template>
    <div>
        <PageTitle title="AniList Profile (Manga)" />
        ...
        <div class="mt-6" v-else-if="userinfo.data">
            ...
            <div class="mt-4">
                <Loading
                    ...
                    text="Loading manga list, please wait..."
                />
                <p
                    class="mt-6 opacity-75 text-center"
                    v-else-if="items.state === 'failed'"
                >
                    Manga list could not be fetched!
                </p>
                <p
                    class="mt-6 opacity-75 text-center"
                    v-else-if="
    ...
    data() {
        const data: {
            ...
            items: StateController<
                NotNull<Await<ReturnType<typeof AniList.mangalist>>>
            >;
            page: number;
            malBaseURL: string;
        } = {
    ...
    methods: {
        ...
        async getItems() {
            try {
                ...
                const items = await AniList.mangalist(
                ...
                this.$logger.emit(
                    "error",
                    `Failed to fetch manga list: ${err?.message}`
                );
            }
        },
        prevPage() {
